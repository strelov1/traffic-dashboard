...
import { registerIngestRoutes } from '../traffic/http/ingest-routes.js'
import { createTrafficRepository } from '../traffic/infra/postgres-repository.js'
import { registerAggregateRoutes } from '../traffic/http/aggregate-routes.js'
...
export function buildServer(
  ...
  const traffic = createTrafficRepository(dependencies.database, {
    // A mutation that could not rebuild its bucket has still happened. The
    // caller is told the truth about the write; the stale total is an operator's
    // problem, so it goes to the log rather than into the response.
    onReconcileFailure: (error: unknown) => {
      server.log.error({ err: error }, 'failed to reconcile the hourly totals')
    },
  })
