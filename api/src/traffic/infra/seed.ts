...
/**
 * Materialises everything below the live tail, once, whatever put it there.
 *
 * Kept out of the seed's empty-table branch on purpose. The aggregate is created
 * with no data, so until it is refreshed the watermark sits at its minimum and
 * every row is served live — the totals look right. When the policy first fires
 * it moves the watermark forward, and everything older than the trailing window
 * becomes neither materialised nor live. A database filled by any route other
 * than the seed would therefore be correct on boot and quietly wrong minutes
 * later, with no error anywhere.
 *
 * Cheap to repeat: the refresh consults the invalidation log, so a boot with
 * nothing new to materialise does nothing.
 */
export async function backfillHourlyTotals(database: Database): Promise<void> {
