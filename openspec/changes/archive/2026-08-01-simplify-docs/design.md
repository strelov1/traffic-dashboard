...
**The main spec is left for `archive` to update, as every other pending change leaves it.** Correcting it here as well is the tempting shortcut — the tree then reads as true before the change lands — and it breaks the only operation the delta exists for: the RENAMED resolves against the main spec's current heading, so a main spec that already carries the new title has no source for it to find, and `archive` aborts with `archive_spec_update_failed` having written nothing. Validation does not catch this, because a delta is validated on its own shape rather than against the spec it will be applied to.
